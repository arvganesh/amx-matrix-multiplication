#include <chrono>
#include <fstream>
#include <iostream>
#include <vector>
#include <string>
#include <optional>
#include <cmath>
#include <map>
#include <algorithm>
#include <numeric>
#include <assert.h>

class Stat {
    std::string _name;
    std::vector<double> _values;
    bool _sorted = false;

    void sort() {
        if(!_sorted) {
            std::sort(_values.begin(), _values.end());
            _sorted = true;
        }
    }

public:
    Stat(): _name("") {}
    Stat(std::string name): _name(name) {}

    void add(double x) {
        _sorted = false;
        _values.push_back(x);
    }

    double num_values() const {
        return _values.size();
    }

    double avg() const {
        assert(_values.size() > 0);
        return std::accumulate(_values.begin(), _values.end(), 0.0) / _values.size();
    }

    double stddev() const {
        double avg = this->avg();
        double sumdiff = 0.0;
        for(const double& x : _values) {
            sumdiff += (x - avg) * (x - avg);
        }
        return sqrt(sumdiff / (_values.size() - 1));
    }

    double percentile(double p) {
        assert(p >= 0 && p <= 1);
        assert(_values.size() > 0);

        sort();

        int idx = round(_values.size() * p);
        return _values[idx];
    }

    double median() {
        return percentile(0.5);
    }

    std::vector<double>& getValues() {
        return _values;
    }

    friend std::ostream& operator<<(std::ostream& os, const Stat& s) {
        os << "Stats for " << s._name << ": n=" << s.num_values() << ", avg=" << s.avg() << ", stddev=" << s.stddev();
        return os;
    }

};

class Timer {
    using clock = std::chrono::high_resolution_clock;
    using time_point_t = std::chrono::time_point<clock>;

    std::string _name;
    time_point_t _start;
    Stat *_stats;

public:
    Timer(std::string name, Stat* stats): _name(name), _start(clock::now()), _stats(stats) {}
    Timer(std::string name) : Timer(name, nullptr) {}
    ~Timer() {
        time_point_t end = clock::now();
        double duration = std::chrono::duration<double>(end - _start).count();
        if(_stats) {
            _stats->add(duration);
        } else {
            std::cout<<"Timer " << _name << ": " << duration << "ms" << std::endl;
        }
    }
};

class Counter {
    std::string _name;
    long _count;
    Stat *_stats;

public:
    Counter(std::string name, Stat* stats): _name(name), _count(0), _stats(stats) {}
    Counter(std::string name): _name(name), _count(0), _stats(nullptr) {}

    void add() {
        ++_count;
    }

    void add_if(bool p) {
        if(p)
            ++_count;
    }

    ~Counter() {
        if(_stats) {
            _stats->add(_count);
        } else {
            std::cout<<"Counter " << _name << " occured " << _count << "times" << std::endl;
        }
    }
};

typedef std::map<std::string, Stat> Stats;

void dumpStatsToCSV(Stats& stats) {
    auto end = std::chrono::high_resolution_clock::now();
    std::string filename = std::to_string(end.time_since_epoch().count()) + "_results.csv";
    std::ofstream of(filename);
    for (auto& kernel_data : stats) {
        of << kernel_data.first << std::endl;
        Stat kernelStats = kernel_data.second;
        auto kernelStatsVec = kernelStats.getValues();
        of << kernelStatsVec[0];
        for (int i = 1; i < kernelStatsVec.size(); ++i) {
            of << ", " << kernelStatsVec[i];
        }
        of << std::endl;
    }
    of.close();
    std::cout << "Wrote to " << filename << std::endl;
}
